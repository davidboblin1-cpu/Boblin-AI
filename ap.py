        ...
        if st.button("✨ Exécuter", use_container_width=True):
            if not image_prompt.strip():
                ...
                with st.spinner("Boblin AI crée votre image..."):
                    try:
                        if uploaded_image:
                            # Modification d'image avec Imagen / Gemini
                            response = client.models.generate_content(
                                model='gemini-2.5-flash',
                                contents=[input_image, image_prompt]
                            )
                        else:
                            # Génération d'image pure via Imagen 3
                        ...
                        # Si c'est une réponse textuelle/multimédia de modification
                        if hasattr(response, 'text') and response.text:
                            st.write(response.text)
                        
                        # Recherche d'images dans la réponse de modification le cas échéant
                        has_img = False
