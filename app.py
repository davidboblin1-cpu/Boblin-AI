            ...
            try:
                ...
                if "429" in error_str or "503" in error_str or "RESOURCE_EXHAUSTED" in error_str or "quota" in error_str.lower() or "unavailable" in error_str.lower():
                    ...
                    if attempt == num_keys - 1:
                        raise Exception("Les serveurs Gemini sont saturés ou les quotas sont atteints. Veuillez patienter un instant.")
                else:
                    raise e
        raise Exception("Erreur de rotation des clés.")

    # -----------------------------------------------------
    # MODE 1 : STUDIO IMAGE & SÉCURITÉ ANTI-ERREUR 503
    # -----------------------------------------------------
        ...
        if st.button("✨ Exécuter", use_container_width=True):
            if not image_prompt.strip():
                ...
                with st.spinner("Boblin AI conçoit et génère votre image..."):
                    try:
                        refined_prompt = ""
                        # Tentative d'appel à Gemini avec gestion de secours si 503
                        try:
                            if uploaded_file:
                                contents = [
                                    input_image, 
                            ...
                            # Si Gemini plante (503 / surcharge), on utilise directement le texte de l'utilisateur pour ne pas bloquer
                            refined_prompt = image_prompt + ", high quality anime style, highly detailed"

                        if not refined_prompt:
                            refined_prompt = image_prompt

                        # Génération et affichage direct de l'image
