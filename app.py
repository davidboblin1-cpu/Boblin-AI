...
if not api_keys_list:
    ...
    def call_gemini_with_rotation(contents_data, is_chat=False, chat_history=None, response_modalities=None):
        ...
        for attempt in range(num_keys):
            ...
            try:
                ...
                # Configuration des modalités (TEXT et IMAGE)
                config = {}
    ...
    # -----------------------------------------------------
    # MODE 1 : STUDIO IMAGE & RETOUCHE MULTIMODALE
    # -----------------------------------------------------
    if app_mode == "🎨 Studio Image":
        st.markdown("### 🎨 Studio Image & Animation")
        st.caption("Générez une image ou transformez une photo en mode animé.")
        ...
        image_prompt = st.text_area("Que voulez-vous créer ou transformer en animé ?", placeholder="Ex: Transforme cette photo en un magnifique dessin animé style manga...")

        if st.button("✨ Exécuter", use_container_width=True):
            if not image_prompt.strip():
                st.warning("Veuillez entrer une description.")
            else:
                with st.spinner("Boblin AI transforme votre image en cours..."):
                    try:
                        # Si l'utilisateur a uploadé une photo, on l'envoie avec le texte
                        if uploaded_file:
                            contents = [input_image, f"{image_prompt}. Redessine et génère entièrement cette image modifiée en mode animé."]
                        else:
                            contents = image_prompt

                        # On demande explicitement au modèle de renvoyer une IMAGE
                            ...
                            for candidate in response.candidates:
                                if candidate.content and candidate.content.parts:
                                    for part in candidate.content.parts:
                                        # Vérification si le bloc contient une image générée
                                        if getattr(part, 'inline_data', None) and part.inline_data.data:
                                            image_bytes = part.inline_data.data
                                            generated_image = Image.open(io.BytesIO(image_bytes))
                                            st.image(generated_image, caption="✨ Résultat animé par Boblin AI", use_container_width=True)
                                            image_displayed = True
                                        elif getattr(part, 'text', None):
                                            # Affiche les commentaires textuels s'il y en a
                                            st.write(part.text)

                        if not image_displayed and hasattr(response, 'text') and response.text:
                            st.write(response.text)
                        elif not image_displayed:
                            st.warning("Le modèle n'a pas renvoyé d'image directe. Essayez de reformuler votre demande de transformation.")
