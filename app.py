...
# ---------------------------------------------------------
# CHARGEMENT DES CLÉS API & LOGIQUE
# ---------------------------------------------------------
    ...
    if app_mode == "🎨 Studio Image":
        ...
        if st.button("✨ Exécuter", use_container_width=True):
            if not image_prompt.strip():
                ...
                with st.spinner("Boblin AI prépare votre prompt visuel..."):
                    try:
                        response = client.models.generate_content(
                            model='gemini-3.6-flash',
                            contents=f"Génère un prompt descriptif ultra-détaillé et professionnel en anglais pour un générateur d'images basé sur cette demande : {image_prompt}"
                        )
