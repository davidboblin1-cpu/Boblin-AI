...
# ---------------------------------------------------------
# INTÉGRATION DE VOS PUBLICITÉS ADSTERRA
# ---------------------------------------------------------
...
# 2. Native Banner avec conteneur
native_banner_code = """
...
# 3. Script publicitaire supplémentaire en bas de page
bottom_script_code = """
...
# Affichage de la bannière Adsterra en haut
...
if not api_keys_list:
    st.info("Veuillez ajouter vos clés API dans les Secrets de Streamlit (ex: GEMINI_API_KEY1, GEMINI_API_KEY2...).")
else:
    def call_gemini_with_rotation(contents_data, is_chat=False, chat_history=None):
        """Tente d'appeler l'API en faisant tourner les clés en cas d'erreur de quota (429)"""
        ...
        for attempt in range(num_keys):
            ...
            try:
                ...
                error_str = str(e)
                # Si l'erreur est liée au quota (429 / Resource Exhausted), on bascule sur la clé suivante
                if "429" in error_str or "RESOURCE_EXHAUSTED" in error_str or "quota" in error_str.lower():
                    st.session_state.key_index = (st.session_state.key_index + 1) % num_keys
                    if attempt == num_keys - 1:
                        raise Exception("Toutes les clés API configurées ont atteint leur quota limite. Veuillez patienter ou ajouter de nouvelles clés.")
                else:
                    raise e
    ...
    if app_mode == "🎨 Studio Image":
        st.markdown("### 🎨 Studio Image")
        st.caption("Générez des descriptions et des concepts visuels détaillés.")

        image_prompt = st.text_area("Décrivez l'image que vous souhaitez concevoir :", placeholder="Ex: Un terrain de football futuriste...")

        if st.button("✨ Exécuter", use_container_width=True):
            if not image_prompt.strip():
                st.warning("Veuillez entrer une description.")
            else:
                with st.spinner("Boblin AI prépare votre prompt visuel (avec rotation des clés)..."):
                    try:
                        prompt_text = f"Génère un prompt descriptif ultra-détaillé et professionnel en anglais pour un générateur d'images basé sur cette demande : {image_prompt}"
                        response = call_gemini_with_rotation(prompt_text, is_chat=False)
                        
                        if response and hasattr(response, 'text'):
                            st.success(f"Prompt généré avec succès ! (Clé n°{st.session_state.key_index + 1})")
                            st.markdown(f"**Description détaillée prête à l'emploi :**")
                            st.code(response.text, language="markdown")
                            st.info("Vous pouvez copier ce prompt dans un générateur d'images gratuit (comme Microsoft Copilot / Bing Image Creator) pour obtenir un résultat parfait.")
                        else:
                            st.warning("Impossible de générer la description. Veuillez réessayer.")
