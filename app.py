...
st.markdown("""
    <style>
        ...
        /* Correction pour TOUTES les zones de saisie */
...
# ---------------------------------------------------------
# CONFIGURATION DES PUBLICITÉS ADSTERRA
# (Mettez vos vrais codes ici ou laissez vide si vous préférez)
# ---------------------------------------------------------
social_bar_script = """
...
# Affichage de la Native Banner en haut
...
if not api_keys_list:
    ...
    # -----------------------------------------------------
    # MODE 1 : STUDIO IMAGE (Corrigé avec Gemini Flash)
    # -----------------------------------------------------
    if app_mode == "🎨 Studio Image":
        st.markdown("### 🎨 Studio Image")
        st.caption("Générez des images par description textuelle.")

        image_prompt = st.text_area("Décrivez l'image que vous souhaitez générer :", placeholder="Ex: Un chat astronaute sur Mars...")
        ...
        if st.button("✨ Exécuter", use_container_width=True):
            if not image_prompt.strip():
                ...
                with st.spinner("Boblin AI crée votre image..."):
                    try:
                        # Utilisation de Gemini 2.5 Flash compatible Developer API pour générer des images
                        ...
                        if not image_found:
                            st.info(response.text if hasattr(response, 'text') else "Aucune image retournée.")
...
# Affichage de la bannière classique en bas
st.markdown("---")
