...
# --- API KEYS & ROTATION SETUP (CORRIGÉ POUR VOS 14 CLÉS) ---
api_keys = []
for i in range(1, 16):
  # Vos secrets sont nommés GEMINI_API_KEY1, GEMINI_API_KEY2, etc. sans underscore avant le chiffre
...
if not client:
  error_config_msg = (
      "⚠️ Veuillez configurer vos clés API (`GEMINI_API_KEY1`, `GEMINI_API_KEY2`, etc.) dans les"
      " secrets de Streamlit."
      if is_french
      else "⚠️ Please configure your API keys (`GEMINI_API_KEY1`, `GEMINI_API_KEY2`, etc.) in"
      " Streamlit secrets."
  )
          ...
          if is_video_request:
            ...
            with st.spinner(spinner_text):
              ...
              response = current_client.models.generate_content(
                  model="gemini-2.5-flash", contents=full_context
              )
              reply = response.text
              st.markdown(reply)
