...
if not api_keys_list:
    ...
    # -----------------------------------------------------
    # MODE 1 : STUDIO IMAGE & MODIFICATION ROBUSTE
    # -----------------------------------------------------
    if app_mode == "🎨 Studio Image":
        ...
        image_prompt = st.text_area("Décrivez l'image ou la modification souhaitée :", placeholder="Ex: Fait une photo de Ronaldo en dessin...")

        if st.button("✨ Exécuter", use_container_width=True):
            if not image_prompt.strip():
                st.warning("Veuillez entrer une description.")
            else:
                with st.spinner("Boblin AI conçoit et génère votre image avec Gemini 3.6 Flash..."):
                    try:
                        # Étape 1 : Obtenir un prompt ultra propre via Gemini 3.6 Flash
                        if input_image:
                            contents = [
                                input_image, 
                            ...
                            contents = f"Rédige un prompt descriptif court, propre et ultra-détaillé en anglais pour : {image_prompt}. Réponds uniquement avec le prompt en anglais, sans caractères spéciaux superflus."
                        ...
                        # Nettoyage des retours à la ligne ou guillemets superflus
                        refined_prompt = refined_prompt.replace('"', '').replace('\n', ' ')

                        if not refined_prompt:
                            refined_prompt = image_prompt

                        # Étape 2 : Utilisation d'une méthode de secours multi-URL si le serveur d'image principal subit une erreur 500
                        ...
                        for img_url in urls_to_try:
                            try:
                                ...
                                with urllib.request.urlopen(req, timeout=20) as response_img:
                        ...
                        if final_image is None:
                            raise Exception(f"Le service de génération d'image est temporairement surchargé (Erreur 500). Veuillez réessayer dans quelques secondes.")
