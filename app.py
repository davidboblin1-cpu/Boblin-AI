...
# ---------------------------------------------------------
# STYLE CSS POUR FIXER LES PUBS ET LA ZONE DE CHAT
# ---------------------------------------------------------
st.markdown("""
    <style>
        /* Style pour la bannière de pub du haut fixe */
        .fixed-top-ad {
            position: sticky;
            top: 0px;
            z-index: 999;
        ...
        /* Style pour la bannière de pub du bas fixe */
        .fixed-bottom-ad {
            position: sticky;
            bottom: 60px; /* Ajusté pour laisser de la place au chat_input */
            z-index: 999;
            background-color: var(--background-color, #0e1117);
            padding: 5px 0;
...
# ---------------------------------------------------------
# EN-TÊTE ET BIENVENUE (PAGE PRINCIPALE)
# ---------------------------------------------------------
st.title("🤖 Boblin AI")
st.write(f"Bonjour **{user_name}** ! Je suis **Boblin AI**, votre assistant virtuel. Que souhaitez-vous faire aujourd'hui ?")

# 1ère Bannière Adsterra FIXÉE EN HAUT
st.markdown('<div class="fixed-top-ad">', unsafe_allow_html=True)
adsterra_code_top = """
...
"""
components.html(adsterra_code_top, height=90)
...
if api_keys_list:
    ...
    # 2ème Bannière Adsterra FIXÉE EN BAS (juste au-dessus du chat)
    st.markdown('<div class="fixed-bottom-ad">', unsafe_allow_html=True)
    adsterra_code_bottom = """
    <script async="async" data-cfasync="false" src="https://pl31362196.profitableratecpmnetwork.com/9c55e3342fc4121417b9d5bc6db0edfc/invoke.js"></script>
    <div id="container-9c55e3342fc4121417b9d5bc6db0edfc"></div>
    """
    components.html(adsterra_code_bottom, height=90)
    st.markdown('</div>', unsafe_allow_html=True)
