...
if not api_keys_list:
    ...
    # -----------------------------------------------------
    # MODE 1 : STUDIO IMAGE AVEC GOOGLE IMAGEN 3
    # -----------------------------------------------------
    if app_mode == "🎨 Studio Image":
        st.markdown("### 🎨 Studio Édition & Génération d'Image")
        st.caption("Générez des images de haute qualité directement via Google Imagen 3.")
        ...
        if st.button("✨ Exécuter", use_container_width=True):
            if not image_prompt.strip():
                ...
                with st.spinner("Boblin AI génère votre image avec Google Imagen 3..."):
                    try:
                        current_key = api_keys_list[st.session_state.key_index]
                        client = genai.Client(api_key=current_key)

                        # Raffinement optionnel du prompt en anglais via Gemini 3.6 Flash
                        if input_image:
                            contents = [
                                input_image, 
                        ...
                        # Génération directe via le modèle natif Imagen 3 de Google
                        ...
                        for generated_image in result.generated_images:
                            final_image = Image.open(io.BytesIO(generated_image.image.image_bytes))
                            st.success("Image générée avec succès !")
                            st.image(final_image, caption="✨ Résultat généré par Boblin AI & Google Imagen", use_container_width=True)
                            break
