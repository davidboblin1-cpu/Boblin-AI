...
# ---------------------------------------------------------
# GESTION DES COMPTES ET CONNEXION UTILISATEUR (SIDEBAR)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# NAVIGATION & ETAT SESSION (SIDEBAR)
# ---------------------------------------------------------
st.sidebar.markdown("### 🛠️ Fonctionnalités")
...
# ---------------------------------------------------------
# EN-TÊTE ET PUBS FIXES ( HAUT )
# ---------------------------------------------------------
st.title("🤖 Boblin AI")
st.write(f"Bonjour **{user_name}** ! Je suis **Boblin AI**, votre assistant virtuel.")
...
# ---------------------------------------------------------
# FONCTIONNALITÉS (ZONE PRINCIPALE)
# ---------------------------------------------------------
if api_keys_list:
    # 1. TEXTE / CHAT
    if mode == "Texte / Chat":
    ...
    # 2. GÉNÉRATION IMAGE (Corrigé pour utiliser les modèles compatibles avec les clés dev standard)
    elif mode == "Générer une Image":
        st.subheader("🖼️ Création d'Image")
        image_prompt = st.text_area("Décrivez l'image à générer :", placeholder="Un jeune homme en costume...")
        
        if st.button("Générer l'Image", type="primary"):
            if image_prompt.strip():
                with st.spinner("Boblin AI génère votre image..."):
                    try:
                        def call_image_gen(client):
                            return generate_text_smart(
                                client, 
                                f"Génère une description ultra-détaillée et visuelle pour créer cette image : {image_prompt}", 
                                system_instruction
                            )
                        response = execute_with_key_rotation(call_image_gen)
                        st.write("### Description détaillée générée par l'IA :")
                        st.write(response.text)
    ...
    # 3. MODIFICATION / ANALYSE D'IMAGE
        ...
        edit_prompt = st.text_area("Votre consigne de modification :", placeholder="Ex: Change l'arrière-plan...")
                ...
                with st.spinner("Boblin AI traite votre image..."):
                    try:
                        ...
                        def call_vision_edit(client):
                            return generate_text_smart(
                                client, 
                                [f"Analyse cette image et réponds à la consigne de modification : {edit_prompt}", image], 
                                system_instruction
                            )
    ...
    # Bannière Adsterra du Bas (visible en permanence)
    st.markdown("---")
    components.html(adsterra_code, height=90)
else:
