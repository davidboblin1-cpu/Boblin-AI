...
if not api_keys_list:
    ...
    # -----------------------------------------------------
    # MODE 1 : STUDIO IMAGE & MODIFICATION
    # -----------------------------------------------------
    if app_mode == "🎨 Studio Image":
        st.markdown("### 🎨 Studio Édition & Génération d'Image")
        st.caption("Importez une photo pour la transformer ou décrivez une création originale.")
        ...
        image_prompt = st.text_area("Décrivez l'image ou la modification souhaitée :", placeholder="Ex: Transforme cette photo en dessin animé détaillé...")

        if st.button("✨ Exécuter", use_container_width=True):
            if not image_prompt.strip():
                st.warning("Veuillez entrer une description.")
            else:
                with st.spinner("Boblin AI analyse et génère votre image avec Gemini 3.6 Flash..."):
                    try:
                        # Étape 1 : Utilisation de Gemini 3.6 Flash pour analyser la photo et générer un prompt descriptif précis en anglais
                        if input_image:
                            contents = [
                                input_image, 
                                f"Analyse cette photo et décris en détail la personne ou le sujet pour créer un prompt artistique en anglais qui applique cette demande : '{image_prompt}'. Le résultat doit conserver les traits principaux du visage/sujet de la photo. Réponds UNIQUEMENT avec le prompt descriptif en anglais."
                            ]
                        else:
                        ...
                        # Étape 2 : Génération de l'image via l'API Pollinations avec un délai de sécurité et encodage propre
                        encoded_prompt = urllib.parse.quote(refined_prompt)
                        image_url = f"https://image.pollinations.ai/prompt/{encoded_prompt}?width=1024&height=1024&seed=1337&nologo=true"
                        ...
                        # Tentative de récupération de l'image avec sécurité
                        with urllib.request.urlopen(req, timeout=30) as response_img:
                            content_type = response_img.headers.get('Content-Type', '')
                            image_bytes = response_img.read()
                        
                        if 'image' not in content_type.lower() and len(image_bytes) < 1000:
                            raise Exception("Le serveur distant n'a pas renvoyé une image valide. Veuillez réessayer.")
