...
# ---------------------------------------------------------
# LECTURE ET ROTATION DES CLÉS API
# ---------------------------------------------------------
api_keys_list = []
...
# ---------------------------------------------------------
# FONCTION POUR GÉNÉRER DU CONTENU EN TESTANT PLUSIEURS MODÈLES
# ---------------------------------------------------------
MODEL_CANDIDATES = [
    "gemini-2.5-flash",
    "gemini-1.5-flash",
...
# ---------------------------------------------------------
# MESSAGE DE BIENVENUE (COURT & CLAIR)
# ---------------------------------------------------------
st.title("🤖 Boblin AI")
...
system_instruction = (
    "Tu es Boblin AI, un modèle d'intelligence artificielle interactif, similaire à des assistants comme ChatGPT ou Gemini. "
    "Ton nom est Boblin AI. Si l'utilisateur te demande qui tu es, explique que tu es Boblin AI, "
...
if api_keys_list:
    ...
    # 1. TEXTE / CHAT
    if mode == "Texte / Chat":
        user_prompt = st.text_area("Votre message :", value=st.session_state.user_input, key="chat_input", placeholder="Qui es-tu ?")
                ...
                with st.spinner("Boblin AI réfléchit..."):
                    try:
                        def call_chat(client):
                            return generate_text_smart(client, user_prompt, system_instruction)
