...
if not api_keys_list:
    ...
    if st.session_state.show_history:
        ...
        # Affichage de l'historique des messages de la conversation en cours
        ...
        for msg in chat_history_data:
            ...
            with st.chat_message(role):
                ...
                st.write(msg_text)
                if msg_text.startswith("[IMAGE_GEN]:"):
                    img_prompt_val = msg_text.replace("[IMAGE_GEN]:", "").strip()
                    encoded_prompt = urllib.parse.quote(img_prompt_val)
                    image_url = f"https://pollinations.ai/p/{encoded_prompt}?width=1024&height=1024&nologo=true&seed=42"
                    st.image(image_url, caption=f"✨ {img_prompt_val}", use_container_width=True)

        # Zone de saisie unifiée unique en bas de page
        user_prompt = st.chat_input("Posez votre question ou tapez 'crée une image de...'...")
        ...
        if user_prompt:
            ...
            with st.chat_message("assistant"):
                with st.spinner("Boblin AI réfléchit..."):
                    try:
                        # Détection automatique si l'utilisateur demande une image
                        lower_prompt = user_prompt.lower()
                        is_image_request = any(keyword in lower_prompt for keyword in ["crée une image", "genere une image", "génère une image", "dessine", "image de"])

                        if is_image_request:
                            # Utilisation de Gemini pour affiner le prompt de l'image en anglais
                            contents = f"Rédige un prompt descriptif court et détaillé en anglais pour : {user_prompt}. Réponds uniquement avec le prompt en anglais."
                            ...
                            ai_response_text = f"Voici l'image générée pour : {user_prompt}"
                            st.write(ai_response_text)
                            
                            encoded_prompt = urllib.parse.quote(refined_prompt)
                            image_url = f"https://pollinations.ai/p/{encoded_prompt}?width=1024&height=1024&nologo=true&seed=42"
                            st.image(image_url, caption=f"✨ {user_prompt}", use_container_width=True)
                            
                            # Sauvegarde propre dans l'historique avec balise image
