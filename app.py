...
# Bouton avec le petit crayon dans le menu latéral
st.sidebar.button("✏️ Nouvelle discussion", on_click=new_chat, use_container_width=True)

# ---------------------------------------------------------
# COMPTE UTILISATEUR ET BIENVENUE
# ---------------------------------------------------------
user_email = getattr(st.experimental_user, "email", None) or getattr(st.user, "email", "Invite")
user_name = getattr(st.experimental_user, "name", "Visiteur")
