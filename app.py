            ...
            if not image_prompt.strip():
                ...
                with st.spinner("Boblin AI prépare votre image..."):
                    try:
                        ...
                        # Téléchargement direct des octets de l'image pour éviter les erreurs de chargement d'URL distante dans st.image
                        ...
                        if img_response.status_code == 200:
                            image_bytes = Image.open(io.BytesIO(img_response.content))
                            st.success("Image générée avec succès !")
                            st.image(image_bytes, caption="✨ Résultat généré par Boblin AI", use_container_width=True)
                        else:
                            st.error("Impossible de récupérer l'image générée.")
