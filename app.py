...
if not api_keys_list:
    ...
    def call_gemini_with_rotation(contents_data, is_chat=False, chat_history=None, response_modalities=None):
        ...
        for attempt in range(num_keys):
            ...
            try:
                ...
                config = {"system_instruction": system_instruction} if is_chat else {}
    ...
    # -----------------------------------------------------
    # MODE 1 : STUDIO IMAGE (Génération & Modification d'images)
    # -----------------------------------------------------
    if app_mode == "🎨 Studio Image":
        st.markdown("### 🎨 Studio Image & Retouche")
        st.caption("Générez une image à partir d'un texte ou importez une photo à modifier/analyser.")

        uploaded_file = st.file_uploader("Importer une photo (optionnel pour modification/analyse) :", type=["png", "jpg", "jpeg"])
        
        if uploaded_file:
            input_image = Image.open(uploaded_file)
            st.image(input_image, caption="Photo importée", use_container_width=True)

        image_prompt = st.text_area("Que voulez-vous créer ou modifier ?", placeholder="Ex: Transforme ce dessin en style manga coloré, ou génère un paysage de football...")

        if st.button("✨ Exécuter", use_container_width=True):
            if not image_prompt.strip():
                st.warning("Veuillez entrer une description ou une instruction.")
            else:
                with st.spinner("Boblin AI traite votre image (avec rotation des clés)..."):
                    try:
                        ...
                        # On autorise le retour d'image et de texte
                                ...
                                if candidate.content and candidate.content.parts:
                                    for part in candidate.content.parts:
                                        if getattr(part, 'inline_data', None):
                                            ...
                                            st.image(generated_image, caption="Résultat par Boblin AI", use_container_width=True)
                        ...
                        if not image_displayed:
                            if hasattr(response, 'text') and response.text:
                                ...
                                st.warning("Le modèle n'a pas renvoyé d'image directement. Essayez de reformuler.")
