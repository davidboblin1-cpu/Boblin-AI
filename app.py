...
def get_full_chat_context(user_email):
    """Récupère le contexte du compte actif pour conserver la mémoire des échanges."""
...
# Code Adsterra officiel mis à jour
adsterra_code = """
...
system_instruction = (
    ...
    "une intelligence artificielle créée pour l'aider à répondre à ses questions, rédiger du texte et analyser des images."
)
...
if api_keys_list:
    ...
    # 1. TEXTE / CHAT
    if mode == "Texte / Chat":
        user_prompt = st.text_area("Votre message :", value=st.session_state.user_input, key="chat_input", placeholder="Posez votre question...")
    ...
    # 2. GÉNÉRATION IMAGE
        ...
        if st.button("Générer l'Image", type="primary"):
            if image_prompt.strip():
                with st.spinner("Boblin AI prépare votre description visuelle..."):
                    try:
                        def call_image_fallback(client):
                            return generate_text_smart(
                                client, 
                                f"Génère une description détaillée, artistique et un prompt optimisé pour une image représentant : {image_prompt}", 
                                system_instruction
                            )
                            
                        response = execute_with_key_rotation(call_image_fallback)
                        st.success("Prompt détaillé généré avec succès :")
                        st.write(response.text)
                        
                        save_to_history(user_email, f"[Image] {image_prompt}", response.text)
                    except Exception as e:
    ...
    # 3. MODIFICATION / ANALYSE D'IMAGE
        ...
        st.subheader("🎨 Retouche et Analyse d'Image")
        uploaded_image = st.file_uploader("Téléchargez l'image :", type=["jpg", "jpeg", "png", "webp"])
        edit_prompt = st.text_area("Votre consigne :", placeholder="Décris cette image...")
                ...
                with st.spinner("Boblin AI traite votre image..."):
                    try:
                        ...
                        def call_vision(client):
                            return generate_text_smart(client, [edit_prompt, image], system_instruction)
                            
                        response = execute_with_key_rotation(call_vision)
                        st.subheader("Résultat de Boblin AI :")
                        st.write(response.text)
                        save_to_history(user_email, f"[Image] {edit_prompt}", response.text)
                    except Exception as e:
                        st.error(f"Erreur : {e}")
            else:
