...
if "show_history" not in st.session_state:
    st.session_state.show_history = false if "show_history" not in st.session_state else st.session_state.show_history
...
if not api_keys_list:
    ...
    def call_gemini_with_rotation(contents_data, is_chat=False, chat_history=None, config=None):
        ...
        for attempt in range(num_keys):
            ...
            try:
                ...
                error_str = str(e)
                if any(err in error_str for err in ["429", "503", "RESOURCE_EXHAUSTED", "quota", "unavailable", "rate limit"]):
                    st.session_state.key_index = (st.session_state.key_index + 1) % num_keys
                    if attempt == num_keys - 1:
                        raise Exception("Toutes les clés API configurées sont actuellement saturées. Veuillez patienter un instant.")
                else:
                    raise e
        raise Exception("Erreur de rotation des clés.")

    # -----------------------------------------------------
    # MODE 1 : STUDIO IMAGE & MODIFICATION DIRECTE
    # -----------------------------------------------------
    if app_mode == "🎨 Studio Image":
        st.markdown("### 🎨 Studio Édition & Génération d'Image")
        st.caption("Importez une photo pour la modifier ou décrivez une création originale.")

        uploaded_file = st.file_uploader("Importer une photo à modifier :", type=["png", "jpg", "jpeg"])
        
        input_image = None
        if uploaded_file:
            input_image = Image.open(uploaded_file)
            st.image(input_image, caption="Photo originale importée", use_container_width=True)

        image_prompt = st.text_area("Que voulez-vous modifier ou créer ?", placeholder="Ex: Modifie cette photo pour ajouter un coucher de soleil cyberpunk en arrière-plan...")

        # Option de choix pour l'utilisateur
        generation_type = st.radio("Type de traitement :", ["Modification directe par l'IA (Multimodal)", "Génération artistique basée sur l'idée"], horizontal=True)

        if st.button("✨ Exécuter la modification", use_container_width=True):
            if not image_prompt.strip():
                st.warning("Veuillez entrer une description de la modification.")
            else:
                with st.spinner("Boblin AI analyse et modifie votre image avec Gemini 3.6 Flash..."):
                    try:
                        if input_image and generation_type == "Modification directe par l'IA (Multimodal)":
                            # Envoi direct de l'image et du prompt à Gemini 3.6 Flash
                            contents = [
                                input_image,
                                f"Agis en tant qu'éditeur d'image expert. Modifie ou décris précisément la modification demandée sur cette image : {image_prompt}."
                            ]
                            response = call_gemini_with_rotation(contents, is_chat=False)
                            
                            if response and hasattr(response, 'text') and response.text:
                                st.success("Analyse et modification textuelle / description réussie :")
                                st.write(response.text)
                            else:
                                st.warning("Le modèle n'a pas renvoyé de texte textuel, vérifiez la sortie.")
                        else:
                            import urllib.parse
                            import urllib.request
                            
                            # Mode génération / transformation par prompt raffiné
                            if input_image:
                                contents = [
                                    input_image, 
                            ...
                            response = call_gemini_with_rotation(contents, is_chat=False)
                            refined_prompt = response.text.strip() if (response and hasattr(response, 'text')) else image_prompt
                            ...
                            if 'image' not in content_type.lower():
                                raise Exception("Erreur lors de la récupération de l'image générée.")
                            
                            final_image = Image.open(io.BytesIO(image_bytes))
                            st.success("Image modifiée/générée avec succès !")
                            st.image(final_image, caption="✨ Résultat modifié par Boblin AI", use_container_width=True)
                            
                    except Exception as e:
                        st.error(f"Erreur de modification : {e}")

    # -----------------------------------------------------
    # MODE 2 : CHAT TEXTE (AVEC GESTION D'IMAGES AUSSI SI BESOIN)
    # -----------------------------------------------------
    else:
        if st.session_state.show_history:
            ...
            user_prompt = st.chat_input("Posez votre question ou demandez de l'aide à Boblin AI...")
            ...
            if user_prompt:
                ...
                with st.chat_message("assistant"):
                    with st.spinner("Boblin AI réfléchit (Gemini 3.6 Flash)..."):
                        try:
